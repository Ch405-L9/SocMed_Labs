...
// ============================================================
function classify(task_type_input, platform) {
    ...
    let task_type;
    if (COPY_GEN_TYPES.has(task_type_input))
        task_type = "copy_gen";
    else if (SUMMARY_TYPES.has(task_type_input))
        task_type = "summarization";
    else if (CODE_TYPES.has(task_type_input))
        task_type = "code_and_structured_data";
    else if (ROUTING_TYPES.has(task_type_input))
        task_type = "routing_utility";
    else
        task_type = "copy_gen";
...
// ============================================================
function route(task_type) {
    const model = MODEL_ROUTING[task_type] ?? MODEL_ROUTING.fallback;
    const REASONS = {
        copy_gen: "gemma3:4b — structured copy generation. mistral:latest EXCLUDED: known failure mode (echoes instructions verbatim on profile_build tasks, confirmed 2026-04-20).",
        summarization: "mistral:latest — long-form context compression (7.2B).",
        code_and_structured_data: "qwen2.5-coder:7b — code and structured data tasks.",
        routing_utility: "llama3.2:3b — lightweight fast classification (3B).",
    };
...
// ============================================================
async function executeWithBrandContext(model, userPrompt, platform, fieldType, constraints, additionalContext) {
...
// ============================================================
function guard(content, platform, fieldType) {
    const violations = [];
    const auto_resolved = [];
    ...
    for (const forbidden of FORBIDDEN_NAME_FORMS) {
        if (content.includes(forbidden)) {
            violations.push(`BRAND_NAME: Forbidden form "${forbidden}" — replace with "BADGRTech" (display) or "BADGRTechnologies LLC" (legal).`);
            brandScore -= 20;
        }
    }
    // G3 — Question opener
    ...
    if (!isTikTokBioException) {
        ...
        if (/^[^.!]*\?/.test(firstLine)) {
            violations.push(`QUESTION_OPENER: Content opens with a question. Rewrite as statement-first.`);
            brandScore -= 15;
            copyScore -= 20;
        }
    }
    ...
    if (platformLimits) {
        ...
        if (limit !== undefined && sanitized.length > limit) {
            violations.push(`CHAR_LIMIT: ${sanitized.length} chars — exceeds ${platform} ${fieldType} limit of ${limit}. Trim required.`);
            compliScore -= 30;
        }
    }
    ...
    if (percentMatches.length > 0) {
        violations.push(`STATS_REVIEW: Percentage stat(s) found: ${percentMatches.join(", ")} — no verified client data exists. Replace with "results vary" or process-proof language.`);
        brandScore -= 25;
    }
    ...
    if (suspiciousDollars.length > 0) {
        violations.push(`STATS_REVIEW: Dollar amount(s) outside $197–$500 range: ${suspiciousDollars.join(", ")} — verify source or replace.`);
        brandScore -= 25;
    }
    // G7 — Fluff phrases
...
// ============================================================
server.registerTool("badgr_pipeline", {
    ...
    try {
        ...
        // ── STAGE 3: TOOL EXECUTION ───────────────────────────
        const rawOutput = await executeWithBrandContext(routing.model, input, platform, field_type, constraints, additional_context);
        ...
        // ── STAGE 4: GUARD ────────────────────────────────────
        const guardResult = guard(rawOutput, platform ?? "generic", field_type ?? "post");
        trace.stage_4_guard = {
...
// ============================================================
server.registerTool("ollama_rag_build", {
...
// ============================================================
server.registerTool("ollama_rag_query", {
    title: "RAG Query",
    description: [
