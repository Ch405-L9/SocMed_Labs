...
function chunkByHeadings(text, source) {
...
function chunkByChunkBlocks(text) {
...
// ── Index build ────────────────────────────────────────────────────────────────
export async function buildIndex(ollama) {
    ...
    for (let i = 0; i < raw.length; i++) {
        ...
        const vec = Array.isArray(res.embeddings?.[0])
            ? res.embeddings[0]
            : [];
        chunks.push({
            id: `chunk_${i}`,
...
function loadIndex() {
    ...
    if (!existsSync(INDEX_PATH))
        return null;
    try {
        _cache = JSON.parse(readFileSync(INDEX_PATH, "utf8"));
        return _cache;
    }
    catch {
        return null;
...
export async function queryRag(ollama, query, topK = 5, filters = {}) {
    const index = loadIndex();
    if (!index)
        return [];
    const res = await ollama.embed({ model: index.embed_model, input: query });
    const qvec = Array.isArray(res.embeddings?.[0])
        ? res.embeddings[0]
        : [];
    if (qvec.length === 0)
        return [];
